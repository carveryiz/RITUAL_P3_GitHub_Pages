...
const state={time:1.2,playing:true,last:performance.now(),selected:6,view:'full',dirty:false,drag:null};
...
function updateReadout(){
  const bar=barAtTime(),beat=Math.floor((state.time%2)/.5)+1,sec=sectionAt(),base=active('base'),behavior=active('behavior'),wall=active('wall'),aux=active('aux');
...
$('#playBtn').onclick=()=>{state.playing=!state.playing;$('#playBtn').classList.toggle('paused',!state.playing);$('#runtimeState').textContent=state.playing?'RUNNING':'PAUSED'};$('#stopBtn').onclick=()=>{state.playing=false;state.time=0;$('#playBtn').classList.add('paused');$('#runtimeState').textContent='STOPPED'};$('#prevBar').onclick=()=>seekBar(barAtTime()-1);$('#nextBar').onclick=()=>seekBar(barAtTime()+1);
...
window.addEventListener('resize',()=>{resize(canvas);resize(signalCanvas)});buildTimeline();syncInspector();requestAnimationFrame(frame);
